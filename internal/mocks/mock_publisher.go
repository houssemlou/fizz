...
// Publish provides a mock function with given fields: ctx, topic, key, value, headers
func (_m *MockPublisher) Publish(ctx context.Context, topic string, key string, value any, headers map[string]string) error {
	ret := _m.Called(ctx, topic, key, value, headers)
	...
	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, string, string, any, map[string]string) error); ok {
		r0 = rf(ctx, topic, key, value, headers)
	} else {
		r0 = ret.Error(0)
	}
...
func (_e *MockPublisher_Expecter) Publish(ctx interface{}, topic interface{}, key interface{}, value interface{}, headers interface{}) *MockPublisher_Publish_Call {
	return &MockPublisher_Publish_Call{Call: _e.mock.On("Publish", ctx, topic, key, value, headers)}
}

func (_c *MockPublisher_Publish_Call) Run(run func(ctx context.Context, topic string, key string, value any, headers map[string]string)) *MockPublisher_Publish_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].(string), args[2].(string), args[3].(any), args[4].(map[string]string))
	})
	return _c
}
...
func (_c *MockPublisher_Publish_Call) RunAndReturn(run func(context.Context, string, string, any, map[string]string) error) *MockPublisher_Publish_Call {
	_c.Call.Return(run)
	return _c
}
